...
export default function Sidebar() {
  return (
    <div className="flex flex-col bg-[#00162E] h-screen w-64 overflow-y-auto gap-6">
      ...
      <div className="flex-1 overflow-y-auto pb-6">
        <div className="flex flex-col gap-1">
          ...
          <span className="pl-6 text-white text-sm font-semibold mt-3">학생 관리</span>
          {tabList
            .filter(item => item.section === "student-manage")
            .map(item => (
              <TabLink key={item.id} item={item} />
            ))}
          <span className="pl-6 text-white text-sm font-semibold mt-3">소통 관리</span>
          {tabList
            .filter(item => item.section === "communication-manage")
            .map(item => (
              <TabLink key={item.id} item={item} />
            ))}
          <span className="pl-6 text-white text-sm font-semibold mt-3">상담 관리</span>
          {tabList
            .filter(item => item.section === "consulting-manage")
            .map(item => (
              <TabLink key={item.id} item={item} />
            ))}
          <span className="pl-6 text-white text-sm font-semibold mt-3">학원비 관리</span>
          {tabList
